...
def make_model(arch: str = "resnet18", num_classes: int = NUM_CLASSES,
    ...
    Trained from scratch (``weights=None``). ``dropout`` > 0 inserts functional
    dropout before ``fc``; being functional it adds no state-dict keys, so the
    weights stay a drop-in for the server's stock ResNet (where dropout is absent,
    and identity at eval regardless).
    """
